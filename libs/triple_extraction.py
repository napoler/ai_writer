        ...
        for index in range(len(postags)):
            ...
            if tmp == 1:
                ...
                # 抽取以谓词为中心的事实三元组
                    ...
                    # print('words',words[index],index)
                    
#                     if 'ATT' in child_dict:
#                         r = words[index]
#                         print(index)
#                         e1 = self.complete_e(words, postags, child_dict_list, child_dict['ADV'][0])
#                         e2 = self.complete_e(words, postags, child_dict_list, child_dict['WP'][0])
#                         svos.append([e1, r, e2])                   
...
#                     if child_dict:
# #                          'words',words[index]
#                         Branchs.append([e1,e2])

                    # #所有的枝干
                    if child_dict:
                        n = ''
                        for it in child_dict:
                            # print('it',it)
                            e1 = self.complete_e(words, postags, child_dict_list, child_dict[it][0])
                            n = n + e1
                        
                        Branchs.append([(words[index],n)])
                    else:
                        Branchs.append([(words[index],'')])
                    # #自定义抽取
                    # if 'ADV' in child_dict and 'VOB' in child_dict and 'COO' in child_dict:
                    #     r = words[index]
                    #     e1 = self.complete_e_plus(words, postags, child_dict_list, child_dict['ADV'][0])
                        
                    #     e2 = self.complete_e_plus(words, postags, child_dict_list, child_dict['VOB'][0])
                    #     e3 = self.complete_e_plus(words, postags, child_dict_list, child_dict['COO'][0])
                    #     print('主谓宾')
                    #     my.append([e1, r, e2,e3])
                    ...
                    if 'SBV' in child_dict and 'CMP' in child_dict:
                        ...
                        if 'POB' in child_dict_list[cmp_index]:
                            ...
                            print('含有介宾关系的主谓动补关系')
                            svos.append([e1, r, e2])
        return svos,Branchs,my

    '''对找出的主语或者宾语进行扩展'''
    def complete_e(self, words, postags, child_dict_list, word_index):
        child_dict = child_dict_list[word_index]
        # print('complete_e的依存',child_dict)
        prefix = ''
        if 'ATT' in child_dict:
    ...
    '''对语句进行扩展'''
    def complete_e_plus(self, words, postags, child_dict_list, word_index):
        child_dict = child_dict_list[word_index]
        print('complete_e的依存',child_dict)
        prefix = ''
        if 'ATT' in child_dict:
        ...
        if 'COO' in child_dict:
            postfix = postfix+ self.complete_e(words, postags, child_dict_list, child_dict['COO'][0])

        return prefix + words[word_index] + postfix
    
    '''对语句进行扩展'''
    def complete_e_p(self, words, postags, child_dict_list, word_index):
        child_dict = child_dict_list[word_index]
        print('complete_e的依存',child_dict)
        prefix = ''
        if 'ATT' in child_dict:
...
#             print('postags', postags)
            ...
            svo,Branch,my = self.ruler2(words, postags, child_dict_list, arcs, roles_dict)
            i= 0
#             for w in words:
# #                 print((i,w))
#                 word_index =i
#                 complete_one = self.complete_e(words, postags, child_dict_list,word_index)
#                 print(complete_one)
...
#                 i = i+1
            ...
            Branchs += Branch
            mys +=my
        return svos,Branchs,mys
    def triples_try(self, content):
        sentences = self.split_sents(content)
        svos = []
...
#                 complete += complete_one
#                 i = i+1
            svos.append((words, postags, child_dict_list, roles_dict, arcs))
